...
def validate_tool_call(payload: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    ...
    if raw_k is None:
        ...
        if isinstance(raw_k, bool):  # bool subclasses int IMPORTANT
            errors.append(
                f"'k' must be an integer, got bool ({raw_k!r}); using default 3"
            )
            coerced_k = 3
            ...
            try:
                ...
                errors.append(
                    f"'k' could not be coerced to int ({raw_k!r}); using default 3"
                )
                coerced_k = 3
            ...
            errors.append(
                f"'k' has unexpected type {type(raw_k).__name__!r}; using default 3"
            )
            coerced_k = 3
...
if __name__ == "__main__":
    cases = [
        ...
        (
            "padded whitespace action",
            {"action": "  answer  "},
            {"action": "answer"},
            [],
        ),
        (
            "newline-padded action",
            {"action": "search\n", "q": "hi"},
            {"action": "search", "q": "hi"},
            [],
        ),
        ...
        (
            "k way below range",
            {"action": "search", "q": "hi", "k": -99},
            {"action": "search", "q": "hi", "k": 1},
            ["'k' must be in [1, 5], got -99; clamping to range"],
        ),
        (
            "k as False — bool trap",
        ...
        (
            "unknown key reasoning",
            {"action": "search", "q": "hi", "reasoning": "because"},
            {"action": "search", "q": "hi"},
            [],
        ),
        (
            "duplicate-ish key k2",
            {"action": "search", "q": "hi", "k": 2, "k2": 5},
            {"action": "search", "q": "hi", "k": 2},
            [],
        ),
        # ── missing fields ───────────────────────────────────────────────────
        ("empty payload", {}, {}, ["Missing required field: 'action'"]),
        (
            "missing action",
            {"q": "hello", "k": 2},
            {},
        ...
        # ── k as float string ────────────────────────────────────────────────────────
        (
            "k as float string '3.0'",
            {"action": "search", "q": "hi", "k": "3.0"},
            {"action": "search", "k": 3, "q": "hi"},
            ["'k' could not be coerced to int ('3.0'); using default 3"],
        ),
        (
            "k as float string '1.0'",
            {"action": "search", "q": "hi", "k": "1.0"},
            {"action": "search", "k": 3, "q": "hi"},
            ["'k' could not be coerced to int ('1.0'); using default 3"],
        ),
        (
            "k as negative float string",
            {"action": "search", "q": "hi", "k": "-1.0"},
            {"action": "search", "k": 3, "q": "hi"},
            ["'k' could not be coerced to int ('-1.0'); using default 3"],
        ),
        # ── llm over-structures values ───────────────────────────────────────────────
        (
            "action nested in dict",
            {"action": {"value": "search"}, "q": "hi"},
            {},
            ["'action' must be a string, got dict"],
        ),
        (
            "q as dict",
            {"action": "search", "q": {"text": "hi"}},
            {},
            ["'q' must be a string, got dict"],
        ),
        (
            "k wrapped in list",
            {"action": "search", "q": "hi", "k": [3]},
        ...
        # ── llm double-wraps payload ─────────────────────────────────────────────────
        (
            "payload wrapped in outer dict",
            {"payload": {"action": "search", "q": "hi"}},
            {},
            ["Missing required field: 'action'"],
        ),
        (
            "payload wrapped as 'data'",
            {"data": {"action": "search", "q": "hi"}},
            {},
            ["Missing required field: 'action'"],
        ),
        # ── llm adds type annotations ────────────────────────────────────────────────
        (
            "action + type key",
            {"action": "search", "q": "hi", "type": "tool_call"},
            {"action": "search", "q": "hi"},
            [],
        ),
        (
            "action + role key",
            {"action": "answer", "role": "assistant"},
            {"action": "answer"},
            [],
        ),
        (
            "action + id key",
            {"action": "search", "q": "hi", "id": "call_abc123"},
            {"action": "search", "q": "hi"},
            [],
        ),
        ...
        (
            "q with unicode",
            {"action": "search", "q": "café résumé"},
            {"action": "search", "q": "café résumé"},
            [],
        ),
        (
            "q with newline inside",
            {"action": "search", "q": "hello\nworld"},
            {"action": "search", "q": "hello\nworld"},
            [],
        ),
        ...
        (
            "action with unicode lookalike",
            {"action": "ѕеаrсh"},
            {},
            ["'action' must be 'search' or 'answer', got 'ѕеаrсh'"],
        ),
        (
            "q is only unicode whitespace",
            {"action": "search", "q": "\u00a0"},
            {},
            ["'q' must be a non-empty string"],
        ),
        ...
        (
            "q is very long string",
            {"action": "search", "q": "a" * 10000},
            {"action": "search", "q": "a" * 10000},
            [],
        ),
        # ── extra nested junk ────────────────────────────────────────────────────────
        (
            "params key with nested k",
            {"action": "search", "q": "hi", "params": {"k": 2}},
            {"action": "search", "q": "hi"},
            [],
        ),
        (
            "metadata key",
            {"action": "search", "q": "hi", "metadata": {"source": "llm"}},
            {"action": "search", "q": "hi"},
            [],
        ),
    ]
